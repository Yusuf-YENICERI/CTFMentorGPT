...
const modalsList = ["gpt-3.5-turbo", "gpt-4-1106-preview", "gpt-4"];
...
export interface AuthType {
  ...
  setTextToSummarize: (text_to_summarize: string) => void;
  user: UserType;
}
...
const useAuth = create<AuthType>()(
  persist(
    (set) => ({
      ...
      text_to_summarize: localStorage.getItem("text_to_summarize") || "",
      setToken: (token) => {
        set(
          produce((state) => {
            state.token = token;
      ...
      setTextToSummarize: (text_to_summarize) => {
        set(
          produce((state) => {
            state.text_to_summarize = text_to_summarize;
          })
        );
        localStorage.setItem("text_to_summarize", text_to_summarize);
      },
    }),
    {
      name: "auth",
